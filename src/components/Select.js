...
export default class Select extends Component {
    ...
    setOptionName(optionData){
      ...
      return `<option data-option-id=${id} data-option-name=${name} data-option-price=${price} data-option-stock=${stock}>
                       ${price === 0 ? `${productName} ${name}`: price > 0 ? `${productName} ${name} (+$${formatPriceAddComma(price)}원)` : ''}
                   </option>
                `
    }
    ...
    template(){
      ...
      return`
        <select name="optionSelect">
          <option>선택하세요.</option>
          ${productOptions.map((optionData) => this.setOptionName(optionData)).join('')}
        </select>
        `
    }
    ...
    setEvent(){
      ...
      $optionSelect.addEventListener('change',(e) => {
        ...
        const {optionId, optionStock, optionName, optionPrice} = selectedOption.dataset

       selectedIndex !== 0 && this.props.onChange({
         optionId, 
         optionStock, 
         optionName, 
         optionPrice
